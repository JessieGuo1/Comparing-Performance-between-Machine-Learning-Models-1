...
def neural(x_train_real, x_test_real, y_train, y_test, transform, model):
    ...
    if model == 0:
        ...
        plt.xlabel("False Positive Rate")
        plt.ylabel("True Positive Rate")
        plt.title(f"ROC curve ({type_str}) {str(Neural Network Selected Features)}")
        plt.tight_layout()
        plt.savefig(f"logistic roc {type_str} Neural Network Selected Features.png")
        plt.close()
...
def main():
    ...
    for i in range(NUM_EXPERIMENTS):
        ...
        rmse_te, r2_te, rmse_tr_i, r2_tr_i = selected(x_train, x_test, y_train, y_test, False, 0, selected_features)
        acc_te, auc_te, acc_tr_i, auc_tr_i = selected(x_train, x_test, y_train, y_test, False, 1, selected_features)
        rmse_nn_sel[i], rsquared_nn_sel[i], acc_sel[i], auc_sel[i] = rmse_te, r2_te, acc_te, auc_te
