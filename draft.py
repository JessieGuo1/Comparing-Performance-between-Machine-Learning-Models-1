...
def selected(x_train_real, x_test_real, y_train, y_test, transform, model):
    if transform == True:
        x_train = x_train_real.copy()
        x_test = x_test_real.copy()
        transformer = MinMaxScaler().fit(x_train.iloc[:,1:])
    ...
    if model == 0:
        regr = linear_model.LinearRegression()
        regr.fit(x_train.iloc[:, [5, 7]], y_train)

        y_pred = regr.predict(x_test.iloc[:, [5, 7]])
        rmse = np.sqrt(mean_squared_error(y_test, y_pred))
        rsquared = r2_score(y_test, y_pred)
        ...
        plt.xlabel("True rings")
        plt.ylabel("Predicted rings")
        plt.title(f"Linear: y_true vs y_pred ({type_str}) Selected Features")
        ...
        plt.hist(res, bins=30)
        plt.xlabel("Residual = y_true - y_pred"); plt.ylabel("count")
        plt.title(f"Linear: residuals hist ({type_str}) Selected Features")
        plt.savefig(f"regression resid hist {type_str}_selected_features.png");
        plt.close()
        ...
        plt.xlabel("Fitted")
        plt.ylabel("Residual")
        plt.title(f"Linear: residuals vs fitted ({type_str}) Selected Features")
        plt.savefig(f"regression resid vs fit {type_str}_selected_features.png")
        plt.close()

        return rmseselec, rsquaredselec

    elif model == 1:
        y_train_binary = (np.asarray(y_train) >= 7)
        y_test_binary = (np.asarray(y_test) >= 7) # note this is y true
        regr = linear_model.LogisticRegression(max_iter=1000)
        regr.fit(x_train.iloc[:, [5, 7]], y_train_binary)

        y_probs = regr.predict_proba(x_test.iloc[:, [5, 7]])[:, 1]
        y_pred = (y_probs >= 0.5)
        acc = accuracy_score(y_test_binary, y_pred)
        auc = roc_auc_score(y_test_binary, y_probs)
        ...
        plt.xlabel("False Positive Rate")
        plt.ylabel("True Positive Rate")
        plt.title(f"ROC curve ({type_str}) Selected Features")
        plt.tight_layout()
        plt.savefig(f"logistic roc {type_str} selected features.png")
        plt.close()

        return accselec, aucselec
...
def main():
    ...
    for i in range(NUM_EXPERIMENTS):
        ...
        rmseselec, rsquaredselec = selected(x_train, x_test, y_train, y_test, True, 0)
        accselec, aucselec = selected(x_train, x_test, y_train, y_test, True, 1)
        rmse_sel[i], rsquared_sel[i], acc_sel[i], auc_sel[i] = rmseselec, rsquaredselec, accselec, aucselec
    
        rmse_val, rsquared_val = neural(x_train, x_test, y_train, y_test)
